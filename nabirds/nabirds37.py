...
if __name__ == "__main__":
    ...
    for image_id in image_identifiers:
        ...
        plt.title(
            "Image ID: %s \n Class Label: %s  [ID: %s]"
            % (image_id, class_name, class_label)
        )
        ...
        currentAxis.add_patch(
            plt.Rectangle(
                (bbox_x, bbox_y), bbox_width, bbox_height, facecolor="green", alpha=0.3
            )
        )
        ...
        if has_networkx:
            ...
            positions = nx.spring_layout(
                G, dim=2, k=20.0, pos=initial_positions, fixed=part_data
            )
            ...
            for part_id, part in enumerate(parts):
                ...
                if v:
                    ...
                    label_xy = (
                        label_position[0] * bbox_width + bbox_x,
                        label_position[1] * bbox_height + bbox_y,
                    )
                    plt.annotate(
                        part_names[part_id],
                        xy=(x, y),
        ...
        print(
            ...
            % (image_sizes[image_id][0], image_sizes[image_id][1])
        )
        ...
        label_path.append(
            (class_names[current_class_label], current_class_label)
        )  # add the root node
        label_path.reverse()
        ...
        ri = input(
            "Press Enter to continue. Or press any key and then enter to quit..."
        )
        if ri != "":
            break
        print()
